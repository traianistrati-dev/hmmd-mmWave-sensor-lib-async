...
fn main(){
    ...
    let mut _uart1 = crate::uart1::init(peripherals.UART1, gpio5_usart1_rx, gpio4_usart1_tx);
    ...
    let mut radar = crate::MicrowaveRadarIO::new(_rx, _tx, esp_hal::delay::Delay::new());
    ...
    if radar.begin_config().is_ok() {

        match radar.get_param_value(ParameterID::RangeGate, &mut parser_params)
        {
        ...
        match radar.get_param_value(ParameterID::AbsenseReportDelay, &mut parser_params)
        {
        ...
        if let Ok(Some(radar_tt_00_val)) = radar.get_param_value(ParameterID::TriggerThreshold00, &mut parser_params) {

            let tt00_values: f32 = decode_threschold_value(radar_tt_00_val);

        }

        if let Ok(Some(radar_ht_00_val)) = radar.get_param_value(ParameterID::HoldThreshold00, &mut parser_params) {

            let ht00_values: f32 = decode_threschold_value(radar_ht_00_val);
        }

        _ = radar.end_save_config();
    }
